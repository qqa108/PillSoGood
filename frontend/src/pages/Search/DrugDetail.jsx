...
const DrugImage = styled.div`
    ...
    flex-shrink: 0;
    border: 1px solid ${colors.point4};
    background: url(${(props) => props.$imageUrl || 'default-image-path.jpg'}) lightgray 50% / cover no-repeat;
    margin-bottom: 1rem;
`;
        ...
        <Container>
            ...
            <DrugInfoContainer>
                ...
                {drugDetail.medicineInformation?.map((info) => (
                    <InfoText key={info.informationId || info.information}>- {info.information}</InfoText> // 고유한 key 추가
                ))}
                ...
                {drugDetail.amountProhibition && (
                    <div>
                        ...
                        <InfoText>
                            {drugDetail.amountProhibition.name} - 하루 최대 {drugDetail.amountProhibition.limits}
