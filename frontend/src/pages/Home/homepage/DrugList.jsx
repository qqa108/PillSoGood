...
const DrugList = () => {
    const mediListInfo = useRecoilValue(takingMediListState); // 약물 정보
    ...
    return (
        <div>
                    ...
                    {mediListInfo.length === 0 ? (
                        ...
                        <ScrollableDrugList>
                            {mediListInfo.map((drug, index) => (
                                <DrugButtonContainer key={index} onClick={() => openDrugDetailModal(drug)}>
                                    <DrugButton />
            ...
            {isRegisterModalOpen && <PillCardRegister isModalOpen={isRegisterModalOpen} closeModal={closeModal} />}
        </div>
    );
};
